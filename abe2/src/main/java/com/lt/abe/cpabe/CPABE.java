package com.lt.abe.cpabe;//import cn.hutool.json.JSONObject;
import com.lt.abe.cpabe.AES;
import com.lt.abe.cpabe.TreeNode;
...
public class CPABE {
    ...
    public static void kemEncrypt(byte[] fileBytes, String accessTreeString,
        ...
        // 使用对称密钥进行对称加密
        byte[] symmetricCp = AES.encrypt(symmetricKey, fileBytes);
        ...
        // 将使用的公共参数文件写入密文中，以便解密时只需要密文和私钥，无需公钥
        ctProp.setProperty("ppFile", pkProp.getProperty("ppFile"));

        storePropToFile(ctProp, ctFileName);
    }

    public static byte[] kemDecrypt(String ctFileName, String skFileName) throws GeneralSecurityException, IOException {
        ...
        if (treeOK) {
            ...
            return "抱歉！您不满足访问条件！".getBytes(); // 访问属性不匹配时返回null或适当的错误信息
//            return null; // 访问属性不匹配时返回null或适当的错误信息
        }
        ...
        byte[] symmetricKey = sha256(randomGT.toBytes());
        // 对称解密
        byte[] decryptedData = AES.decrypt(symmetricKey, symmetricCp);
